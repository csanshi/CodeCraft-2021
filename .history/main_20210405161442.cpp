...
string serverModel = "hostATVAZ";
// int purchaseVersion = 1; // 0-只买一种 1-买最便宜
int migrationVersion = 1; // 0-AB无选择地插入，1-AB有选择地插入 2-改成continue & 做两次
...
public:
    ...
    void printInfo(){
        cout << "A: " << core[0] << ' ' << mem[0] << " capacity: " << capacity_core << endl;
        cout << "B: " << core[1] << ' ' << mem[1] << " capacity: " << capacity_mem << endl;
    }
    double getTotalLeft(){
