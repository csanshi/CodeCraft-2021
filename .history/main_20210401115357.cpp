...
unordered_map<int, Position, modified_hash>vmToServer;
...
void buildOutputStream(const vector<PSI>&book, const vector<MigrateInfo>&migmyRationInfo, const vector<Tuple>addInfo, vector<int>&idxForAddInfo){ //
    cout << "(purchase, " << book.size() << ")\n";
    for(auto& kv : book){
        cout << "(" << kv.first << ", " << kv.second << ")\n";
    }

    cout << "(migmyRation, " << migmyRationInfo.size() << ")" << endl;
    for(auto& e : migmyRationInfo){
        if(e.node == 2){
...
public:
    ...
    vector<MigrateInfo>migmyRationInfo;
    ...
    void handleRequestsInOneDay(vector<Request>& requests){
        ...
        this->migmyRationInfo.clear();

        this->migrateByVersion(migmyRationVersion);
        ...
        buildOutputStream(book, this->migmyRationInfo, addInfo, this->idxForAddInfo); //, this->idxForAddInfo
        this->calPowerCost();
    }    
        ...
        for(int i = 0; i < n-1; i++){
            ...
            for(auto& vm : server.vms){
                ...
                if(node == -1){ // 无处可插
                    ...
                    this->migmyRationInfo.push_back({vm.first, this->servers[idx[toServer]].id, node});
