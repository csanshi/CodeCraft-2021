...
public:
    ...
    void handleRequestsInOneDay(vector<Request>& requests){
        if(emptyServers.size()+notEmptyServers.size() != servers.size()){
            cout << emptyServers.size() << ' ' << notEmptyServers.size() << ' ' << servers.size() << endl;
            // assert(emptyServers.size()+notEmptyServers.size() == servers.size());
        }
